...
其实，使用很简单，我们把要加的批注写在 =<div class="org-note">...</div>= 中就可以了，放心，在渲染的时候已经做好了处理，如下：
...
原理就是，在 Org Mode 中输出 HTML 片段，指定了自定义的标签 =<div class="org-note">= （用来后续的 DOM 操作）。在浏览器渲染的过程中，无效的标签符并不会做为普通文本绘制出来，但其内的内容却可以。
